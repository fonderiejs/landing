...
| 5 | **The cost thesis & credibility** — is Fonderie *cheaper* in tokens, and does the brain reliably drive the build? | **Cost goal met (N=3); the one quality gap found is now covered** | Pre-registered benchmark, 36 clean sessions (3 conditions × 3 sequences × a 4-session growing app). Transcript-measured knowledge overhead **pb/fat = 0.24 → a fraction (≤⅓)**, stable as N grew; a parity-plus scare was traced (for $0, by instrumenting transcripts we already had) to turn-count noise that averaged out at N=3. The full 39-point rubric surfaced one real weakness: on **1 of 3** sequences the brain-guided model **stalled** — it asked permission on a feature it could have installed, and shipped incomplete. We did not paper over it: root-caused it, then built **defense-in-depth** — directive guidance, plus a deterministic detect-and-recover loop (inspect the built tree; if the capability is missing, re-invoke; never pass silently) and a completion gate — proven by a $0 forced-stall fixture, not just asserted. From-scratch stayed consistently *complete but insecure* (hardcoded secret 2/3) in ~3× the code. Nothing claimed beyond the measurement | `fonderie-js/experiments/phase41-2026-07/BATCH-RESULTS.md` + `DISCOVERY-RELIABILITY.md`; `BRAIN_PLAN.md` § Phase 4.1 |
