...
| 2 | **Retrieval quality** — naive phrasing ("let people pay") misses the right nodes; keyword search alone scores ~50–76% on public benchmarks | Merged to SDK main (PR #12); official gate pending real corpus | Replaced free-text queries with a closed enum of language-less concept IDs (`billing.subscriptions`, …) — the LLM maps intent to concept in the tool call, so it works identically in every language. Pilot eval: a below-spec model picking from the enum alone scored **96/96** across EN + FR + Romanian (Romanian added with zero code changes) | `fonderie-js/BRAIN_PLAN.md` § "R2 update" + "Pilot run"; merged in fonderiejs/sdk#12 |
