...
| 6 | **Onboarding friction — the database cliff** — a Fonderie app needed an external Postgres (`DATABASE_URL`) before it would run, while a from-scratch app starts standalone; a real first-run wall | Solved | We asked whether *authoring* even needs a database — it doesn't. The bricks are audited and own their schema: migrations ship inside each package and run on boot, every route is guaranteed by the package. So a Fonderie app is **done when it typechecks and is wired per recipe — zero external setup to build**. Confirmed ($0, no model): a typecheck-clean, auth-wired app booted, self-applied 10 migrations, and served `POST /auth/register` → **201 with real JWTs**, no hand-written glue. The database is a *runtime* concern (a real, persistent Postgres via one env var — not the from-scratch in-memory store that loses every account on restart), not an authoring prerequisite. So the "needs a DB" caveat is a run step, not a build wall — and we retired an embedded-DB build we'd been about to spec, because the friction was already gone | `fonderie-js/experiments/phase41-2026-07/DECISION-DB-FREE-AUTHORING.md` |
| 7 | **The clock** — 3 live clients and $3k+ MRR by Nov 2026; one archetype in flight, three remain | In progress | One real paying project per archetype, in strict order A→C→B→D; each phase gated, a gate that fails twice ends the phase | `fonderie-js/ROADMAP.md` |
